...
// Perceptron Banner DOM
const plantStatusBanner = document.getElementById('plant-status-banner');
...
function handleIncomingData(data) {
    ...
    const clase = data.clase !== undefined ? parseInt(data.clase) : null;
    ...
    // Update Perceptron Banner Status
    updatePerceptronBanner(clase);
...
function updateUIState() {
    ...
    if (isSerialConnected) {
        ...
        // Reset Perceptron Banner on disconnect
        updatePerceptronBanner(null);
    }

    updateRecordingUI();
}

// Update the Perceptron Banner based on PIC decision
function updatePerceptronBanner(clase) {
    if (clase === null) {
        plantStatusBanner.className = 'perceptron-banner';
        plantStatusText.textContent = 'Esperando telemetría...';
        return;
    }

    switch (clase) {
        case 0:
            plantStatusBanner.className = 'perceptron-banner state-ok';
            plantStatusText.textContent = '✓ Excelente (La planta está cómoda y en condiciones ideales)';
            break;
        case 1:
            plantStatusBanner.className = 'perceptron-banner state-water';
            plantStatusText.textContent = '💧 Alerta: ¡Falta Riego! (La tierra está seca)';
            break;
        case 2:
            plantStatusBanner.className = 'perceptron-banner state-sun';
            plantStatusText.textContent = '☀️ Alerta: ¡Mucho Sol! (Mucha radiación solar)';
            break;
        default:
            plantStatusBanner.className = 'perceptron-banner';
            plantStatusText.textContent = 'Inferencia desconocida';
    }
}
